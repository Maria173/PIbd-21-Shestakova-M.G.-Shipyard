...
namespace ShipyardClientApp.Controllers
{
    public class HomeController : Controller
    {
        ...
        {
            ViewBag.Ships = APIClient.GetRequest<List<ShipViewModel>>("api/main/getshiplist");
            return View();
        }
        [HttpPost]
        public void Create(int ship, int count, decimal sum)
        {
            if (count == 0 || sum == 0)
            {
                return;
        ...
        [HttpPost]
        public decimal Calc(decimal count, int ship)
        {
            ShipViewModel prod =
            APIClient.GetRequest<ShipViewModel>($"api/main/getship?shipId={ship}");
